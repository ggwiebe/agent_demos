...
# MAGIC %pip install -U -qqqq databricks-agents mlflow-skinny databricks-sdk[openai] tenacity
...
# MAGIC import json
# MAGIC
# MAGIC from tenacity import (
# MAGIC     retry,
# MAGIC     stop_after_attempt,
# MAGIC     wait_random_exponential,
# MAGIC )  # for exponential backoff
# MAGIC
...
# MAGIC                 new_messages.append(asdict(message))
# MAGIC             else:
# MAGIC                 new_messages.append(message)
# MAGIC         messages = new_messages
# MAGIC         # print(messages)
# MAGIC         # print(type(messages))
# MAGIC         # print(type(messages[0]))
...
# MAGIC     ##
# MAGIC     @retry(wait=wait_random_exponential(min=1, max=60), stop=stop_after_attempt(6))
# MAGIC     def chat_completion_with_backoff(self, **kwargs):
# MAGIC         return self.model_serving_client.chat.completions.create(**kwargs)
# MAGIC
...
# MAGIC             self.chat_completion_with_backoff,
# MAGIC             name="chat_completions_api",
# MAGIC             span_type="CHAT_MODEL",
# MAGIC         )
# MAGIC         request = {"messages": messages, "temperature": self.config.get("temperature"), "max_tokens": self.config.get("max_tokens"),  "tools": RETRIEVER_TOOL_SPEC} #, "parallel_tool_calls":False}
# MAGIC         # print(request)
# MAGIC         # print(type)
