...
class FunctionCallingAgent(mlflow.pyfunc.ChatModel):
    ...
    def predict(
        ...
        for message in messages:
            if type(message) == ChatMessage:
                new_messages.append(asdict(message))
            else:
    ...
    def chat_completion_with_backoff(self, **kwargs):
        return self.model_serving_client.chat.completions.create(**kwargs)
    ...
    def chat_completion(self, messages: List[ChatMessage]) -> ChatResponse:
        ...
        request = {"messages": messages, "temperature": self.config.get("temperature"), "max_tokens": self.config.get("max_tokens"),  "tools": RETRIEVER_TOOL_SPEC} #, "parallel_tool_calls":False}
        # print(request)
        # print(type)
