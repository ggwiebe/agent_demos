...
# MAGIC %md
# MAGIC First, we load the documents used by our Agent.
...
# MAGIC %md
# MAGIC Now, we will pass these documents (Databricks docs in this demo) to the Synthetic API along with some guidance to tune the generated questions for this use case.  The API will generate synthetic questions & ground truth responses using our propietary synthetic data pipeline that we built in partnership with Mosaic AI Research.
# MAGIC
# MAGIC Looking at the generated questions, you can see that for each document, we generate a synthetic question and the expected facts (ground truth) that the Agent must generate to get the questions correct.  We generate JUST the facts, rather than a fully written answer, since this [1] helps the accuracy of our propietary LLM judges we will see later and [2] makes it more efficient for your SMEs to review these questions: by having just the facts, versus a generated response, they can quickly review/edit.
...
# MAGIC %md
# MAGIC Now, let's use this synthetic evaluation set to evaluate the quality of an Agent.  To allow us to focus on how Agent Evaluation works, we create a simple Agent with only an LLM - later in this notebook, we will evaluate an Agent with a Retriever. 
# MAGIC
# MAGIC You can see the MLflow Trace to understand what this Agent is doing.  MLflow Traces provide unified observability across development and production for your Agent - this same trace is available for every production request once you deploy the Agent.
...
# MAGIC %md
# MAGIC
# MAGIC Now, let's run Agent Evaluation's propietary LLM judges using the synthetic evaluation set to see the quality/cost/latency of the Agent and identify any root causes of quality issues.  Our research team has invested signficantly in the quality AND speed of these judges.  We optimize the judges for how often the judge agrees with human raters.  Our judges outperform competitors such as RAGAS in terms of their quality and speed. Read more [details in our blog](https://www.databricks.com/blog/databricks-announces-significant-improvements-built-llm-judges-agent-evaluation).  
# MAGIC
# MAGIC Use the MLflow Evaluation UI to inspect the individual records & see the judge outputs, including details about the identified root causes of quality issues.  This UI, coupled with the speed of evaluation, help you efficiently test your hypotheses to improve quality, letting you reach the production quality bar in less time. 
# MAGIC
# MAGIC Use the MLflow Runs UI to compare quality/cost/latency metrics between experiements.  This helps you make informed tradeoffs in partnership with your business stakeholders about cost/latency/quality.  Further, you can use this view to provide quantitative updates to your stakeholders so they can follow your progress improving quality!
