...
doc_paths_to_remove_from_synthetic_generation = [
    "/archive/", "/release-notes/", "/resources/",
    "/sql/language-manual/", "/admin/", "/migration/", "/dev-tools/", "/error-messages/"
]

for pattern in doc_paths_to_remove_from_synthetic_generation:
    CHUNKS = CHUNKS.loc[~CHUNKS["doc_uri"].str.contains(pattern)]

docs_for_synthetic_generation_df = CHUNKS[:500].to_dict("records")
display(docs_for_synthetic_generation_df)
...
mixtral8x7b_config = baseline_config.copy()
# llama405b_config['endpoint_name'] = 'databricks-meta-llama-3-1-405b-instruct'
mixtral8x7b_config['endpoint_name'] = 'databricks-mixtral-8x7b-instruct'

log_and_evaluate_agent(
    agent_config=mixtral8x7b_config,
    run_name="mixtral8x7b",
)

llama70b_config = baseline_config.copy()
llama70b_config['endpoint_name'] = 'databricks-meta-llama-3-1-70b-instruct'
llama70b_config['endpoint_name'] = 'agents-demo-llama-3-1-70b-instruct'
