...
# MAGIC %md
# MAGIC ## 1.3/ Share evaluation set with subject matter experts
# MAGIC
# MAGIC <talk about needs>

# COMMAND ----------

from databricks.agents.evals import datasets as eval_datasets

EVALS_TABLE_NAME = f"agents_demo.synthetic_data.db_docs_eval_set__{user_name}"

# Uncomment if it already exists to reset
eval_datasets.delete_evals_table(EVALS_TABLE_NAME)

# Create managed evaluation table
eval_datasets.create_evals_table(
  evals_table_name=EVALS_TABLE_NAME,
  agent_name="Databricks Docs",  
)

# Import the dataset into managed evals
eval_datasets.add_evals(evals_table_name=EVALS_TABLE_NAME, evals=evals)

# Get links to UIs
eval_datasets.get_ui_links(evals_table_name=EVALS_TABLE_NAME,)
